...
Bird::Bird() : Agent(-10, 10, 4, 720, 1.2, 4, 270, 0.4, AgentType::Neutral, Color::Blue), m_cohesion(1.0), m_alignment(3.0), m_separation(1.0), m_avoidance(1.0), m_hunting(1.0){}; // View Angle 270° Par défault

Bird::Bird(const Vec2& position, const Vec2& velocity) : 
	Agent(position, velocity, 5, 270, 100, 1, 1, 0.4, AgentType::Neutral, Color::Blue), m_cohesion(1.0), m_alignment(3.0), m_separation(1.0), m_avoidance(1.0), m_hunting(1.0){};  // Range = Cohesion Range = Allignment Range = 30 
...
Bird::Bird(const Vec2& position, const Vec2& velocity, const int& bodySize, const int& viewAngle, const int& range, const float& minSpeed, const float& maxSpeed,  const float cohesionWeight, const float alignmentWeight, const float separationWeight, const float avoidanceWeight, const float huntingWeight):
	Agent(position, velocity, bodySize, viewAngle, range, minSpeed, maxSpeed, 0.4, AgentType::Neutral, Color::Blue), m_cohesion(cohesionWeight), m_alignment(alignmentWeight), m_separation(separationWeight), m_avoidance(avoidanceWeight), m_hunting(huntingWeight){
};
