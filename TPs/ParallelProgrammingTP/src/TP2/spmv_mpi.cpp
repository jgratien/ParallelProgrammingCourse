  ...
  else
  {
    ...
    if (my_rank == 0) { // Noeud Maitre
      ...
      SPARSE MATRIX VECTOR MPI */
      {
        ...
        // SEND SIZES OF DATA
        int from(row_off[0]); // From index
        int to(row_off[tab_local_sizes[0]] - 1);
        std::vector<int> tab_nb_vals(nb_proc); // enregistrement de nb_vals
        tab_nb_vals[0] = (to - from + 1);
        row_off += tab_local_sizes[0]; // offset du pointeur pour le noeud 1
        std::vector<MPI_Request> requests_nbvals(nb_proc-1); // Requests pour les pacquets asynchrones envoyés
        for (int i=1; i<nb_proc;++i)
        {
          from = row_off[0];
          to = row_off[tab_local_sizes[i]] - 1;
          tab_nb_vals[i] = to - from + 1; // nombres de valeurs à envoyer
          tosend_sizes[0] = (int) tab_local_sizes[i];
          tosend_sizes[1] = to - from + 1;
          MPI_Isend(tosend_sizes.data(), 2, MPI_INT, i /* pour Noeud i */, i /* TAG  */, MPI_COMM_WORLD, &requests_nbvals[i-1]);
          row_off += tab_local_sizes[i]; // offset pour le noeud suivant
        }
        ...
        for (int i=1; i<nb_proc;++i)
        {
          MPI_Isend(row_off, tab_local_sizes[i] + 1, MPI_INT, i, 30, MPI_COMM_WORLD, &requests_rowoff[i-1]) ;
        ...
        MPI_Status st;
        for(auto r : requests_nbvals) { // attendre de recevoir tous les sizes
            MPI_Wait(&r, &st);
        }
        ...
        for (int i = 1; i < nb_proc; ++i) {
          MPI_Isend(col_idx, tab_nb_vals[i], MPI_INT, i , 1000 + i , MPI_COMM_WORLD, &requests_colidx[i-1]);
          MPI_Isend(V_ptr, tab_nb_vals[i], MPI_DOUBLE, i, 666 + i , MPI_COMM_WORLD, &requests_vptr[i-1]);
          col_idx += tab_nb_vals[i]; // décalage pour le noeud suivant
          V_ptr += tab_nb_vals[i]; // décalage pour le noeud suivant
        }

        for(auto r : requests_rowoff) { //attendre de recevoir les row_idx
            MPI_Wait(&r, &st);
        }
        for(auto r : requests_colidx) { //attendre de recevoir les colidx
            MPI_Wait(&r, &st);
        }
        for(auto r : requests_vptr) { // attendre de recevoir les valeurs
            MPI_Wait(&r, &st);
        }
      ...
      MPI_Recv(row_off, local_size + 1, MPI_INT, 0, 30, MPI_COMM_WORLD, &status);
