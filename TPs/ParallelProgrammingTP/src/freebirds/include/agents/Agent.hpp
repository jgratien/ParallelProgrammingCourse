...
class Agent
{
   ...
   protected:
    ...
    Agent(const int& lVelocity, const int& uVelocity, const float weight, const float height, const float ratio, const int& bodySize, const int& viewAngle,  const float turnFactor, AgentType type);
    Agent(const Vec2& position, const Vec2& velocity, const int& bodySize, const int& viewAngle, const int& range, const float& minSpeed, const float& maxSpeed,  const float turnFactor, AgentType type);
