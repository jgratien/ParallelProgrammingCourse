...
Eagle::Eagle() : Agent(-10, 10, 1080, 720, 1.2, 4, 270, 0.4, AgentType::Predator) {};

Eagle::Eagle(const int& lVelocity, const int& uVelocity, const float weight, const float height, const float ratio): 
	Agent(lVelocity, uVelocity, weight, height, ratio, 4, 270, 0.4, AgentType::Predator), m_cohesion(1.0), m_separation(2.0), m_hunting(1.0){};  
...
Eagle::Eagle(const int& lVelocity, const int& uVelocity, const float weight, const float height, const float ratio, const int& bodySize, const int& viewAngle, const float cohesionWeight, const float separationWeight, const float huntingWeight):
	Agent(lVelocity, uVelocity, weight, height, ratio, bodySize, viewAngle, 0.4, AgentType::Predator), m_cohesion(cohesionWeight), m_separation(separationWeight), m_hunting(huntingWeight){};
