...
class CSRMatrix
{
  ...
  public:
    ...
    CSRMatrix(int a, int b){
        // This is a test matrix
        m_kcol = {0,1,3,4,6,7};
        m_cols = {2,0,3,2,0,1,4};
    ...
    void printSize(){
      std::cout << " LocalMatrixSize: " << m_kcol.size() << " " << m_cols.size() << " " << m_values.size() << std::endl;
    }
    ...
    void ompmult(VectorType const& x, VectorType& y) const
    {
      ...
      assert(y.size()>=m_nrows) ;
      {
         ...
         #pragma omp parallel shared(x,y)
           {
             #pragma omp for schedule(dynamic,m_chunk_size) nowait  
             for(std::size_t irow =0; irow<m_nrows;++irow)
             {
               double value = 0 ;
               for( int k = m_kcol[irow]; k < m_kcol[irow+1];++k)
