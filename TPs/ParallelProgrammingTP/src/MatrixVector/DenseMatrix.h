...
namespace PPTP
{
  ...
  void mult(std::vector<double> const& x, std::vector<double>& y, std::size_t nrows, std::size_t ncols, std::vector<double> const& values)
  {
      assert(x.size() >= nrows);
      assert(y.size() >= nrows);
      double const* matrix_ptr = values.data();
